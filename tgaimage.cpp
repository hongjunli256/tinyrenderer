...
TGAColor TGAImage::get(const int x, const int y) const {
    ...
    const std::uint8_t* p = data.data() + (y * w + x) * bpp;
...
void TGAImage::flip_horizontally() {
    for (int i = 0; i < w / 2; i++)
        for (int j = 0; j < h; j++)
            for (int b = 0; b < bpp; b++)
                std::swap(data[(j * w + i) * bpp + b], data[(j * w + w - 1 - i) * bpp + b]);
}
...
void TGAImage::flip_vertically() {
    for (int i = 0; i < w; i++)
        for (int j = 0; j < h / 2; j++)
            for (int b = 0; b < bpp; b++)
                std::swap(data[(j * w + i) * bpp + b], data[((h - 1 - j) * w + i) * bpp + b]);
}
...
void TGAImage::clear(const TGAColor& c) {
    // 如果是 RGB 三通道相同 或 单通道 → 用 memset（极速）
    if (bpp == 1 || (c.bgra[0] == c.bgra[1] && c.bgra[1] == c.bgra[2])) {
        memset(data.data(), c.bgra[0], data.size());
    }
    // 否则只能逐像素设置（颜色不同没办法）
    else {
        for (int j = 0; j < h; j++)
